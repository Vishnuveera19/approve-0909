import { Typography, Grid, FormControl, InputLabel,formData, Card, CardContent, Checkbox, Button } from '@mui/material';
...
export default function Leaveapprovenew0909() {
        ...
        <div>
          <Grid style={{ padding: "80px 5px0 5px" }}>
            <Card style={{ maxWidth: 800, margin: "0 auto" }}>
              <CardContent>
                <Typography variant='h5' color='S- Light' align='center'> Approve</Typography>
                <form>
                                            ...
                                            leaveapply.filter((r)=>r.pnBranchId== pnBranchId && r.status == 'P').map((e,index)=>{
                                                ...
                                                return (
                                                    <tr key={e.pnEmployeeCode}>
                                                        ...
                                                        <td style={{textAlign:'left', padding:'10px', paddingRight:'10px'}}>
                                                        <Button 
  variant='contained' 
  color='success' 
  size='small' 
  disabled={canceledLeaves.includes(e.empCode)}  
  onClick={(event) => {
    postRequest(ServerConfig.url, SAVE, {
      query: `INSERT INTO ApprovalDB (emp_code, emp_name, approved_by, login_by, approve_type, approval_date, Status) 
            VALUES ('${e.empCode}', '${e.empName}', '${e.approvedby}', '${loginBy}', '${e.comments}', '${getCurrentDate()}', 'Approve')`
  }).then((response) => {
    handleApprove(e.empCode, e);
    navigate('/LeaveApproveTable09', { state: { data: e, status: 'Approve' } });
  }).catch((error) => {
      console.error('Error inserting data into ApprovalDB:', error);
      // Handle error here
  })
...
<td style={{textAlign:'left', padding:'10px', paddingLeft:'10px'}}>
<Button variant='contained' color='error' size='small' onClick={() => {
  postRequest(ServerConfig.url, SAVE, {
    query: `INSERT INTO ApprovalDB (emp_code, emp_name, approved_by, login_by, approve_type, approval_date, Status) 
            VALUES ('${e.empCode}', '${e.empName}', '${e.approvedby}', '${loginBy}', '${e.comments}', '${getCurrentDate()}', 'Cancel')`
}).then((response) => {
    handleCancel(e.empCode, e);
    navigate('/LeaveApproveTable09', { state: { data: e, status: 'Cancel' } });
}).catch((error) => {
    console.error('Error inserting data into ApprovalDB:', error);
    // Handle error here
})
